...
// EXTENDED TIMEOUT for ESP32 (user's ESP32 needs 12+ seconds)
#define SCAN_TIMEOUT_MS 25000           // 25 seconds timeout (was 15)
#define SCAN_MIN_TIME_MS 2000           // Minimum 2 seconds before showing results
#define SCAN_FALLBACK_DELAY_MS 18000    // 18 seconds before fallback simulation (was 8)
...
typedef struct {
    ...
    // UART - with extended timeout support
    ...
    // State - with extended timing support
    ...
    bool esp32_response_detected;   // NEW: Track if ESP32 actually responds
    uint32_t last_uart_rx_time;     // NEW: Track last UART RX activity
...
// UART functions with extended timeout
