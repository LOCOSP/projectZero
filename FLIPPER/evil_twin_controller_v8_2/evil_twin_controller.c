...
void safe_shutdown_threads(EvilTwinControllerApp* app) {
    ...
    // Stop UART thread FIRST - before touching mutexes
    if(app->uart_thread) {
...
// Process complete UART line safely - CRITICAL MUTEX HANDLING
...
// UART worker thread with CRITICAL atomic flags coordination
int32_t uart_worker_thread(void* context) {
    SAFE_CHECK_RETURN(context, -1);
    EvilTwinControllerApp* app = context;

    FURI_LOG_I(TAG, "UART worker started with atomic coordination");
    ...
    while(app->uart_thread_running && app->app_running && !app->shutdown_requested) {
        ...
        // Handle scan timeout (only if not shutting down)
        if(app->uart_state == UartStateScanning && !app->shutdown_requested) {
            ...
            if(!app->real_esp32_mode && get_scan_elapsed_ms(app) > 8000 && !app->scan_completed && !app->shutdown_requested) {
...
// App allocation with CRITICAL mutex initialization
EvilTwinControllerApp* evil_twin_controller_app_alloc() {
    ...
    // Initialize ALL fields to safe values - CRITICAL for atomic flags
    ...
    FURI_LOG_I(TAG, "Application allocated successfully with critical mutex safety");
    return app;
}

// App cleanup with CRITICAL shutdown sequence: THREADS BEFORE MUTEXES
...
int32_t evil_twin_controller_app(void* p) {
    ...
    FURI_LOG_I(TAG, "Starting Evil Twin Controller with critical mutex safety");
