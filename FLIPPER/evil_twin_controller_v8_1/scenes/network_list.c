...
// Draw callback for network list with PROPER TIMING LOGIC
static void network_list_draw_callback(Canvas* canvas, void* context) {
    ...
    // FIXED TIMING LOGIC: Show proper scan status
    if(app->uart_state == UartStateScanning) {
        ...
        canvas_draw_str(canvas, 10, 25, "Skanowanie ESP32...");
        ...
        canvas_draw_str(canvas, 10, 35, time_str);
        ...
        if(elapsed_sec < timeout_sec) {
            ...
            canvas_draw_str(canvas, 10, 45, timeout_str);
        } else {
            canvas_draw_str(canvas, 10, 45, "Timeout...");
        }

        canvas_draw_str(canvas, 10, 55, "Czekam na ESP32...");
        ...
        canvas_draw_str(canvas, 10, 25, "TIMEOUT ESP32!");
        canvas_draw_str(canvas, 5, 35, "Sprawdz polaczenie:");
        canvas_draw_str(canvas, 5, 45, "- Pin 13/14 + GND");
        canvas_draw_str(canvas, 5, 55, "- ESP32 wlaczone");
    ...
    if(app->scan_completed && (!app->networks_ready || app->network_count == 0)) {
        ...
        canvas_draw_str(canvas, 5, 45, "ESP32 nie znalazł AP");
        canvas_draw_str(canvas, 2, 63, "Back=Menu");
        return;
    }
    ...
    if(!app->scan_completed && !app->networks_ready && app->uart_state == UartStateIdle) {
        canvas_draw_str(canvas, 15, 35, "Gotowy do skanowania");
        canvas_draw_str(canvas, 5, 45, "Kliknij skanowanie");
        canvas_draw_str(canvas, 2, 63, "Back=Menu");
        return;
    }

    // Networks list - use REAL data with bounds checking
    int y = 20;
    int visible = 0;
...
// Input callback for network list with scan state handling
static bool network_list_input_callback(InputEvent* event, void* context) {
    ...
    if(app->uart_state == UartStateScanning) {
        if(event->type == InputTypeShort && event->key == InputKeyBack) {
            ...
            FURI_LOG_I(TAG, "User cancelled scan");
...
void evil_twin_controller_scene_network_list_on_enter(void* context) {
    ...
    FURI_LOG_I(TAG, "Network list entered - state: %d, scan_completed: %d, networks: %d", 
              app->uart_state, app->scan_completed, app->network_count);
}
