...
// CRITICAL: Safe shutdown functions (same as v8.4)
...
// Check if should start fallback simulation (after 18 seconds)
...
// Force UI refresh when networks are added
void force_ui_refresh(EvilTwinControllerApp* app) {
    ...
    FURI_LOG_D(TAG, "UI refresh triggered");
}
...
void handle_scan_timeout(EvilTwinControllerApp* app) {
    ...
    uint32_t elapsed = get_scan_elapsed_ms(app);
    FURI_LOG_W(TAG, "EXTENDED scan timeout after %u ms (25s limit)", (unsigned int)elapsed);
...
// IMPROVED: SAFE network line parser with BETTER FILTERING
bool parse_network_line_safe(const char* line, NetworkInfo* network) {
    ...
    if(strlen(line) < 20) return false; // Too short to be valid
    ...
    // Skip ALL whitespace (spaces, tabs) - user has many spaces
    while(*data_start && (*data_start == ' ' || *data_start == '\t')) {
        data_start++;
    }

    if(*data_start == '\0') {
        FURI_LOG_D(TAG, "No data after projectZero: marker");
        return false;
    }
    ...
    if(!(*data_start >= '0' && *data_start <= '9')) {
        FURI_LOG_D(TAG, "Skipping non-network line: '%.*s'", 20, data_start);
        return false;  // Not a network line, don't warn
    }

    FURI_LOG_D(TAG, "Parsing ESP32 data: '%s'", data_start);

    // Parse with safe handling of missing SSID
    char temp_bssid[20] = {0};
    char temp_ssid[68] = {0};

    // More flexible parsing - handle multiple spaces
    ...
    if(parsed < 5) {
        FURI_LOG_W(TAG, "Failed to parse ESP32 network line - got %d fields, need 5+", parsed);
        return false;
    }
    ...
    if(network->index < 0 || network->index > 255) {
        FURI_LOG_W(TAG, "Invalid network index: %d", network->index);
        return false;
    }
    if(network->rssi > 0 || network->rssi < -100) {
        FURI_LOG_W(TAG, "Invalid RSSI: %d", network->rssi);
        return false;
    }
    if(network->channel < 1 || network->channel > 200) {
        FURI_LOG_W(TAG, "Invalid channel: %d", network->channel);  
        return false;
    }
    if(network->auth < 0 || network->auth > 10) {
        FURI_LOG_W(TAG, "Invalid auth: %d", network->auth);
        return false;
    }

    FURI_LOG_I(TAG, "✅ Successfully parsed ESP32 network: idx=%d, rssi=%d, ch=%d, ssid='%s'", 
              network->index, network->rssi, network->channel, network->ssid);

    return true;
}

// Process complete UART line safely - WITH IMPROVED ESP32 DETECTION
void process_uart_line_safe(EvilTwinControllerApp* app, const char* line) {
    ...
    if(app->uart_state == UartStateScanning) {
        ...
        if(strstr(line, "Found") && strstr(line, "APs")) {
            FURI_LOG_I(TAG, "ESP32 found networks, clearing old data");
        ...
        if(strstr(line, "Index") && strstr(line, "RSSI") && strstr(line, "BSSID")) {
            FURI_LOG_I(TAG, "ESP32 header line detected, ready for network data");
            force_ui_refresh(app);
            return;
        }

        // Try to parse network line - IMPROVED: Only parse actual network lines
        if(app->network_count < MAX_NETWORKS) {
            ...
            if(parse_network_line_safe(line, &network)) {
                ...
                if(furi_mutex_acquire(app->uart_mutex, 1000) == FuriStatusOk) {
                    ...
                    if(app->network_count < MAX_NETWORKS && !app->shutdown_requested) {
                        ...
                        FURI_LOG_I(TAG, "🎉 Added ESP32 network %d: %s (RSSI: %d) - Total networks: %d", 
                                  network.index, network.ssid, network.rssi, app->network_count);

                        // IMPORTANT: Force UI refresh after adding network
                        force_ui_refresh(app);
                    }
            ...
            // Note: No "Line not recognized" log anymore - reduces spam
        } else {
            FURI_LOG_W(TAG, "Maximum networks reached (%d), ignoring additional networks", MAX_NETWORKS);
        }
        ...
        if(strstr(line, "scan complete") || strstr(line, "scan done") || 
           strstr(line, "Scan finished") || strstr(line, "finnished scan")) {
            ...
            app->scan_completed = true;
            app->esp32_response_detected = true;
            FURI_LOG_I(TAG, "🎉 ESP32 scan completed successfully - Total networks: %d", app->network_count);
            force_ui_refresh(app);
        }
    }

    // Always try to update UI when processing lines
...
// UART worker thread with FIXED FORMATTING and ESP32 detection
int32_t uart_worker_thread(void* context) {
    ...
    while(app->uart_thread_running && app->app_running && !app->shutdown_requested) {
        ...
        if(app->uart_state == UartStateScanning && !app->shutdown_requested) {
            ...
            // Log progress every 5 seconds with correct format
...
// PROBLEM 1 FIX: Send UART command safely and RETURN SUCCESS/FAILURE
bool uart_send_command_safe(EvilTwinControllerApp* app, const char* command) {
    SAFE_CHECK_RETURN(app, false);
    SAFE_CHECK_RETURN(command, false);

    if(app->shutdown_requested) return false;  // Don't send during shutdown

    if(!app->uart_initialized || !app->serial_handle) {
        FURI_LOG_W(TAG, "UART not initialized, cannot send to ESP32: %s", command);
        add_log_line_safe(app, "ERROR: UART not available");
        add_log_line_safe(app, "ESP32 not connected or firmware issue");
...
EvilTwinControllerApp* evil_twin_controller_app_alloc() {
    ...
    app->esp32_response_detected = false;  // ESP32 response tracking
    app->last_uart_rx_time = 0;           // UART RX activity tracking
    ...
    FURI_LOG_I(TAG, "Application allocated with UART + UI fixes v8.5");
    return app;
}

// App cleanup (same as v8.4)
...
int32_t evil_twin_controller_app(void* p) {
    ...
    FURI_LOG_I(TAG, "Starting Evil Twin Controller v8.5 with UART + UI fixes");
    ...
    FURI_LOG_I(TAG, "Evil Twin Controller v8.5 finished successfully");
    return 0;
}
