Evil ESP Controller - Flipper Zero App
=====================================

A Flipper Zero application for controlling the Evil-ESP WiFi deauther module via UART.
...
The Evil ESP Controller allows you to control an Evil-ESP WiFi deauther module directly from your Flipper Zero device. The app provides a user-friendly interface for WiFi scanning, target selection, deauthentication attacks, and device configuration with real-time UART monitoring.
...
• ESP32/ESP8266-based module  
• Evil-ESP firmware by 7h30th3r0n3  
...
```
┌─────────────────┐ ┌─────────────────┐
│ Flipper Zero    │ │ ESP Module      │
├─────────────────┤ ├─────────────────┤
│ Pin 1 (5V)      │────│ 5V              │
│ Pin 13/15 (TX)  │────│ GPIO3 (RX)      │
│ Pin 14/16 (RX)  │────│ GPIO1 (TX)      │
│ Pin 18 (GND)    │────│ GND             │
└─────────────────┘ └─────────────────┘
```
...
2. Navigate to the Evil-ESP directory and build:
```bash
cd Evil-ESP
ufbt
```
...
4. Use bulk operations:
   ...
   ◦ "Confirm Targets" - Send selections to ESP
...
• **Send Config to Device** - Apply all settings to ESP

#### 5. UART Terminal

• Immediate live monitoring of ESP communication
...
The app communicates with the ESP module using these commands:
...
• `set cycle_delay` - Configure cycle delay
• `set scan_time` - Configure scan duration
• `set frames` - Set frames per AP
• `set start_channel` - Set starting channel
...
• Real-time parsing of all ESP response types
...
• Verify wiring: Pin 1→5V, Pin 13/15→GPIO3, Pin 14/16→GPIO1, Pin 18→GND
• Check 115200 baud rate communication
• Ensure ESP module has Evil-ESP firmware flashed
• Try UART terminal to test basic communication

### No Networks Found

• Check ESP module power and connectivity
• Verify antenna connections on ESP
• Check debug log at `/ext/apps_data/evil_esp_debug.txt`
...
• **Original Evil-ESP Firmware**: 7h30th3r0n3
• **Flipper Zero App Development**: dag nazty
• **ESP Hardware**: ESP32/ESP8266-based modules

Special thanks to the Flipper Zero community and WiFi security researchers.

## License

This project is licensed under the MIT License. See the original Evil-ESP project for firmware licensing details.
...
**Hardware Requirements**: Flipper Zero + ESP module with Evil-ESP firmware  
**Original Firmware**: Evil-ESP by 7h30th3r0n3
