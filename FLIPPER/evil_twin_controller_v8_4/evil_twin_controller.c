...
// CRITICAL: Safe shutdown functions (same as v8.3)
...
// NEW: Force UI refresh when networks are added
...
// IMPROVED: SAFE network line parser with BETTER SPACE HANDLING
bool parse_network_line_safe(const char* line, NetworkInfo* network) {
    ...
    // IMPROVED: Skip ALL whitespace (spaces, tabs) - user has many spaces
    while(*data_start && (*data_start == ' ' || *data_start == '\t')) {
        data_start++;
    }

    if(*data_start == '\0') {
        FURI_LOG_W(TAG, "No data after projectZero: marker");
        return false;
    }
    ...
    // IMPROVED: More flexible parsing - handle multiple spaces
    ...
    if(parsed < 5) {
        FURI_LOG_W(TAG, "Failed to parse ESP32 line - got %d fields, need 5+", parsed);
        return false;
    }
...
void process_uart_line_safe(EvilTwinControllerApp* app, const char* line) {
    ...
    if(app->uart_state == UartStateScanning) {
        ...
        // Try to parse network line - THIS IS THE KEY PART
        if(app->network_count < MAX_NETWORKS) {
            ...
            if(parse_network_line_safe(line, &network)) {
                ...
                FURI_LOG_D(TAG, "Line not recognized as network data");
            }
        } else {
...
int32_t uart_worker_thread(void* context) {
    ...
    while(app->uart_thread_running && app->app_running && !app->shutdown_requested) {
        ...
        if(app->uart_state == UartStateScanning && !app->shutdown_requested) {
            ...
            // FIXED: Log progress every 5 seconds with correct format
...
EvilTwinControllerApp* evil_twin_controller_app_alloc() {
    ...
    FURI_LOG_I(TAG, "Application allocated with EXTENDED 25s ESP32 timeout + FIXED parsing");
    return app;
}

// App cleanup (same as v8.3)
...
// Main entry point (same as v8.3)
int32_t evil_twin_controller_app(void* p) {
    UNUSED(p);

    FURI_LOG_I(TAG, "Starting Evil Twin Controller v8.4 with FIXED parsing + UI refresh");
    ...
    FURI_LOG_I(TAG, "Evil Twin Controller v8.4 finished successfully");
    return 0;
}
